...
#define INNERLOOP 2000
#define OUTERLOOP 2000

//defining number of tasks - SZ
...
int main()
{
    ...
    for (i = 0; i < NPERIODICTASKS; i++)
    {
        ...
        printf("sched_get_priority_max(SCHED_FIFO) : %d", sched_get_priority_max(SCHED_FIFO));
        printf("sched_get_priority_min(SCHED_FIFO) : %d", sched_get_priority_min(SCHED_FIFO));
...
void *task1(void *ptr)
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        clock_gettime(CLOCK_REALTIME, &currentTime);
        long int nextArrivalTimeInNano = next_arrival_time[0].tv_nsec + (next_arrival_time[0].tv_sec * 1000000000);
        long int currentTimeInNano = currentTime.tv_nsec + (currentTime.tv_sec * 1000000000);
        printf("To jest arrivalTimeInNano: %ld\n", nextArrivalTimeInNano);
        printf("To jest currentTimeInNano: %ld\n", currentTimeInNano);

        if (nextArrivalTimeInNano < currentTimeInNano)
        {
            missed_deadlines[0]++;
            printf("\nThis is clock_nanosleepfrom task1 %ld\n", nextArrivalTimeInNano - currentTimeInNano);
        }
...
void *task2(void *ptr)
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        int a = clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &next_arrival_time[1], NULL);
        if (a != 0)
        {
            printf("\nThis is clock_nanosleepfrom task1 %d\n", a);
            missed_deadlines[0]++;
        };
...
void *task3(void *ptr)
{
    // set thread affinity, that is the processor on which threads shall run
    cpu_set_t cset;
    __CPU_ZERO_S(sizeof(cpu_set_t), &cset);
    CPU_SET(0, &cset);
    pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cset);
    ...
    for (i = 0; i < 25; i++)
    {
        ...
        int a = clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &next_arrival_time[2], NULL);
        if (a != 0)
        {
            printf("\nThis is clock_nanosleepfrom task1 %d\n", a);
            missed_deadlines[0]++;
